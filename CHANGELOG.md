...
Add changes from upstream real 1.2.2.
...
- New feature: allow arbitrary command-line options to be passed through to
  `restic backup`
  (see <https://github.com/phlummox-patches/backupninja/pull/3>)

### Fixed

- Remove old luks header when doing sys backup
  (fixes <https://0xacab.org/liberate/backupninja/-/issues/11333>)
- Fix non-working duplicity wizard in ninjahelper
  (fixes <https://0xacab.org/liberate/backupninja/-/issues/11336>)
- Fix runtime errors when restic handler with `--debug`
  (fixes <https://0xacab.org/liberate/backupninja/-/issues/11328>)
